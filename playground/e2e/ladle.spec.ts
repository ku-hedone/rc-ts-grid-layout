...
const PLAYGROUND_STORY = `${LADLE_BASE}/?story=playground--playground`;
const MAX_ROWS_STORY = `${PLAYGROUND_STORY}&arg-maxRows=4`;
const DROPPING_STORY = `${LADLE_BASE}/?story=playground--dropping-playground`;
...
function collectProjectErrors(page: Page): string[] {
  const errors: string[] = [];
  page.on('console', (msg) => {
...
test.describe('Ladle - Basic Story', () => {
  ...
  test('鼠标拖拽生效：item 位置改变', async ({ page }) => {
    ...
    if (box) {
      ...
      if (newBox) {
        const moved = Math.abs(newBox.x - initialX) > 5 || Math.abs(newBox.y - initialY) > 5;
  ...
  test('多步拖拽累计位移，不只使用最后一帧 delta', async ({ page }) => {
    ...
    for (let i = 1; i <= steps; i++) {
      await page.mouse.move(startX + (totalX * i) / steps, startY + (totalY * i) / steps);
    }
  ...
  test('maxRows 约束拒绝越界拖拽后恢复到合法 DOM 位置', async ({ page }) => {
    await page.goto(MAX_ROWS_STORY);
    await page.waitForLoadState('networkidle');
  ...
  test('无项目侧 console.error', async ({ page }) => {
    const errors = collectProjectErrors(page);
...
test.describe('Ladle - Dropping Story', () => {
  ...
  test('drop 成功：onDrop 回调触发，文本包含 __dropping-elem__', async ({ page }) => {
    ...
    await page.goto(DROPPING_STORY);
    await page.waitForLoadState('networkidle');
    ...
    const droppedText = page.locator('text=drop-');
  ...
  test('连续 drop 后不残留 dropping placeholder', async ({ page }) => {
    ...
    const dropCount = bodyText.match(/drop-/g)?.length ?? 0;
