...
const GridLayout: FC<RGLProps> = memo(
  ({
    ...
    const containerHeight = useMemo(() => {
      ...
      const containerPaddingY = Array.isArray(containerPadding) ? containerPadding[1] : margin[1];
    ...
    const placeholder = useMemo(() => {
      if (rect) {
        ...
        return (
          ...
          >
            <div key="item-placeholder" {...wrapperProps} />
          </GridItem>
        );
      }
      return null;
    ...
    const onInnerDrop: DragHandler = useCallback(
      ...
      [droppingItem.i, innerLayout, onDrop, preventBrowserNativeAction, removeDroppingPlaceholder],
    );
    // 拖拽离开事件处理
    ...
    const onInnerResize: Required<ItemProps>['onResize'] = useCallback(
      (i, w, h, { e, node, handle }) => {
        ...
        let x;
        let y;
        const [nextLayout, item] = withLayoutItem(innerPropsRef.current.layout, i, (item) => {
          let hasCollisions;
          x = item.x;
          y = item.y;
    ...
    const onInnerResizeStop: Required<ItemProps>['onResizeStop'] = useCallback(
      (i, _w, _h, { e, node }) => {
        const item = getLayoutItem(innerPropsRef.current.layout, i);
        const nextLayout = innerCompactor.compact(innerPropsRef.current.layout, cols);
        const previousItem = old.current;
        lastRect.current = void 0;
        old.current = void 0;
    ...
    const genGridItem = useCallback(
      (child: ReactNode, isDrop?: boolean) => {
        if (child && typeof child === 'object' && 'key' in child) {
          ...
          const draggable =
            typeof item.isDraggable === 'boolean' ? item.isDraggable : !item.static && isDraggable;
          const resizable =
            typeof item.isResizable === 'boolean' ? item.isResizable : !item.static && isResizable;
  ...
  (prev, next) => prev.children === next.children && fastRGLPropsEqual(prev, next, deepEqual),
);
