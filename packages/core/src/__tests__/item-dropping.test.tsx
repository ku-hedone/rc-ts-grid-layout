...
describe('GridItem - Dropping 行为', () => {
  ...
  it('未提供 onDragOver 时使用 defaultItem 创建 dropping item', () => {
    const layout: Layout = [];
    const onDrop = vi.fn();
    ...
    render(
      <GridLayout
        ...
        layout={layout}
        gridConfig={{ cols: 12, rowHeight: 30 }}
        dropConfig={{ enabled: true, defaultItem: { i: '__dropping-elem__', w: 2, h: 2 } }}
        onDrop={onDrop}
      >
        {null}
      </GridLayout>,
    ...
    if (container) {
      ...
      expect(onDrop.mock.calls[0]?.[1]).toMatchObject({
        i: '__dropping-elem__',
        w: 2,
        h: 2,
      });
