...
rc-ts-grid-layout: react-grid-layout 的 TypeScript 函数组件实现。基于上游 react-grid-layout v2 的 TS 版本，移除 class 组件，使用 hooks 重构。
...
保持与上游 react-grid-layout 的 API 兼容性。参考 `git-repo/react-grid-layout/` 中的实现，但使用 TypeScript 和函数组件重写。
...
`git-repo/react-grid-layout/` 是上游库的本地副本（只读参考）。

- 上游已迁移到 TypeScript，采用可组合接口设计（Compactor、PositionStrategy、LayoutConstraint）
- 详细差异分析见 `docs/upstream-diff-analysis.md`
- 修复计划按 P0/P1/P2 优先级执行
